// ==UserScript==
// @name         Torn Multi-Player Watcher v8.2 (Persistent Hidden Watch List + Remove All + Status Colors)
// @namespace    https://www.torn.com/
// @version      0.8.2
// @description  Multi-player Torn watcher with draggable panel, local-only key storage, manual custom-key guidance, browser troubleshooting help, player-name lookup, clearer ID validation, current status display, persistent watched players while hidden, remove-all watched players, and blue/red status colors.
...
(function () {
  ...
  function statusInfoForWatcher(watcher) {
    ...
    if (/travel/i.test(main) || /travel/i.test(state.description || '') || /abroad/i.test(main) || /abroad/i.test(state.description || '')) className += ' tmw82-sub-travel';
    else if (/hospital/i.test(main) || /hospital/i.test(state.description || '')) className += ' tmw82-sub-hospital';
